#!/usr/bin/env python3
# build_ipk.py - Baut ein Enigma2-IPK-Paket fuer MagentaMusik
...
PLUGIN_SRC  = os.path.join(SCRIPT_DIR, "MagentaMusik")
...
def build_data_tar():
    ...
    with tarfile.open(fileobj=buf, mode="w:gz", format=tarfile.GNU_FORMAT) as tar:
        for root, dirs, files in os.walk(PLUGIN_SRC):
            dirs[:] = [d for d in dirs if d not in ("__pycache__", "covers")]
